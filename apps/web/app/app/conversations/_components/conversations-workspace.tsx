...
import { useAIRuntimeStore, useRuntimeConversationDetail } from "@/lib/runtime";
...
export function ConversationsWorkspace({ summaries, activeId, detail: serverDetail }: ConversationsWorkspaceProps) {
  const [mobilePanel, setMobilePanel] = useState<MobilePanel>("list");
  const hydrated = useAIRuntimeStore((s) => s.hydrated);
  const storeSummaries = useAIRuntimeStore((s) => s.conversationSummaries);
  const storeDetail = useRuntimeConversationDetail(activeId);
    ...
    >
      ...
      <div className="relative grid min-h-0 flex-1 md:grid-cols-[minmax(260px,320px)_1fr] lg:grid-cols-[minmax(260px,300px)_1fr_minmax(280px,340px)]">
        <div className={cn("min-h-0 overflow-hidden md:block", showList ? "block" : "hidden")}>
          <ConversationList rows={summariesLive} activeId={activeId} onSelectConversation={openThread} />
        </div>
