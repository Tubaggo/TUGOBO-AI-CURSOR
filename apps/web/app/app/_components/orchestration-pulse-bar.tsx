...
import { OrchestrationPulsePlaceholder } from "./live-ui-placeholder";
...
export function OrchestrationPulseBar() {
  ...
  const focusItems = mounted ? metrics.focusItems : [];
  const currentFocus =
    focusItems[focusIndex % Math.max(focusItems.length, 1)]?.label ?? "Monitoring operations";

  useEffect(() => {
    if (!mounted || focusItems.length <= 1) return;
    const id = window.setInterval(() => {
      setFocusIndex((i) => (i + 1) % focusItems.length);
    }, ROTATE_MS);
    return () => window.clearInterval(id);
  }, [mounted, focusItems.length]);

  useEffect(() => {
    if (!mounted || pulse <= 0 || focusItems.length <= 1) return;
    setFocusIndex((i) => (i + 1) % focusItems.length);
  }, [mounted, pulse, focusItems.length]);

  if (!mounted) {
    return <OrchestrationPulsePlaceholder />;
  }
  ...
  return (
    <div className="hidden items-center gap-2 sm:flex">
      ...
      <span className="hidden max-w-[220px] flex-col gap-0.5 xl:flex 2xl:max-w-[280px]">
