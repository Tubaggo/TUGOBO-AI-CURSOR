...
export function buildCognitionSnapshot(
  ...
  if (conversation.flags.paymentRisk) {
    interpretation =
      "Guest wants to complete the booking, but payment friction is pushing them toward abandonment.";
  } else if (conversation.flags.humanTakeover) {
    interpretation =
      "VIP policy dispute needs a human decision; full booking and revenue context is ready for staff.";
  } else if (conversation.flags.recoveryActive) {
    interpretation =
      "Recovery is underway — alternate payment route sent while €" +
      `${directValue.toLocaleString("en-EU")} remains exposed.`;
  } else if (conversation.status === "resolved") {
    interpretation = "Booking is firm; post-confirmation upsell and pre-arrival nurture remain open.";
  } else if (conversation.unread > 0) {
    interpretation = "New inquiry on direct channel — respond with availability before OTA leakage.";
  }
  ...
  let recommendedAction = "Monitor thread and respond within SLA.";
  if (conversation.flags.paymentRisk) {
    recommendedAction = "Send one-click payment route with reduced checkout friction.";
  } else if (conversation.flags.humanTakeover) {
    recommendedAction = "Let staff close; keep payment link and policy notes visible.";
  } else if (conversation.flags.otaConversion) {
    recommendedAction = "Offer direct rate match before OTA commission is lost.";
  } else if (conversation.status === "resolved" && guest) {
    recommendedAction = "Offer late checkout or breakfast bundle — guest accepts post-confirmation.";
  } else if (conversation.unread > 0) {
    recommendedAction = "Send availability and direct rate in guest language.";
  }
